...
register(
    "BrittleStarGymEnv-v0",
    entry_point="brittle_star_gym_environment:BrittleStarGymEnv"
)
...
class Args:
    ...
    env_id: str = "BrittleStarGymEnv-v0"
...
def create_envs(num_envs, base_seed):
    env_fns = [make_env(base_seed + i) for i in range(num_envs)]
...
class Actor(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        # Predict the mean for each of the 20 action dimensions
        mean = nn.Dense(self.action_dim, kernel_init=initializers.orthogonal(0.01), bias_init=initializers.constant(0.0))(x)

        # Predict the log standard deviation for each action dimension
        log_std = nn.Dense(self.action_dim, kernel_init=initializers.constant(0.0))(x)
...
if __name__ == "__main__":
    ...
    print("Create envs")
    # envs = create_envs(num_envs=args.num_envs, base_seed=args.seed)
    env = BrittleStarGymEnv()
    ...
    print("Create network, actor and critic")
    network = Network()
    ...
    # todo: with init they were reset
    # next_obs = jnp.stack([jnp.array(env.reset()[0]) for env in envs])
    next_obs = env.reset()
    ...
    def step_once(carry, step, env_step_fn):
        ...
        return ((agent_state, episode_stats, next_obs, next_done, key), storage)

    def rollout(agent_state, episode_stats, next_obs, next_done, key, step_once_fn, max_steps):
        (agent_state, episode_stats, next_obs, next_done, key), storage = jax.lax.scan(
            step_once_fn, (agent_state, episode_stats, next_obs, next_done, key), (), max_steps
        )

        return agent_state, episode_stats, next_obs, next_done, storage, key
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        agent_state, episode_stats, next_obs, next_done, storage, key = rollout(
            agent_state, episode_stats, next_obs, next_done, key
        )
